...
export default function BookingFilters({
  ...
  const handleSearchChange = (value: string) => {
    onQ(value);
    if (onApplyFilters) {
      setTimeout(() => onApplyFilters(), 300); // Debounce search
    }
  };
  ...
  const handleSlotTypeChange = (value: SlotTypeFilter) => {
    onSlotType(value);
    if (onApplyFilters) {
      setTimeout(() => onApplyFilters(), 0);
    }
  ...
  const hasActiveFilters =
    q !== "" || buildingId !== "ALL" || roomId !== "ALL" || slotType !== "ALL";
      ...
      <div className="rounded-2xl border border-gray-200/60 dark:border-gray-800/60 bg-white dark:bg-gray-900/40 backdrop-blur-sm shadow-lg shadow-gray-200/20 dark:shadow-none p-6">
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              placeholder="Search by room no or room name..."
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <div className="relative">
              <Clock className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-gray-400 pointer-events-none z-10" />
              <select
        ...
        {hasActiveFilters && (
          <div className="mt-5 flex flex-wrap items-center gap-2 pt-5 border-t border-gray-200 dark:border-gray-800">
            ...
            {slotType !== "ALL" && (
              <div className="inline-flex items-center gap-2 rounded-lg bg-orange-100 dark:bg-orange-500/20 px-3 py-1.5 text-xs font-medium text-orange-700 dark:text-orange-300 border border-orange-200 dark:border-orange-500/30">
                <Clock className="h-3 w-3" />
                <span>
                  {slotOptions.find((s) => s.id === slotType)?.name ||
                    "Slot Type"}
                </span>
                <button
                  onClick={() => handleSlotTypeChange("ALL")}
                  className="ml-1 rounded-md hover:bg-orange-200 dark:hover:bg-orange-500/30 p-0.5 transition-colors"
                >
                  <X className="h-3 w-3" />
                </button>
              </div>
